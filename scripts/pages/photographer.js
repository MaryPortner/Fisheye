...
export function photographerTemplateSingle(data) {
    ...
    function getUserSingleCardDOM() {
        ...
        for (let i = 0 ; i < data.length ; i ++){
            ...
            presentationPhotographer.appendChild(photographer.taglineText);

            // On crée un lien sur l'élément image
            const linkImgPhotographe = document.createElement('a');
            linkImgPhotographe.setAttribute("title", `page de ${photographer.name}`);
            linkImgPhotographe.setAttribute("href", "");

            const imgPhotographerHomePage = document.createElement('img');
            imgPhotographerHomePage.setAttribute("src", photographer.picture); // On lui met la src qui provient de la constante picture
            imgPhotographerHomePage.dataset.id = id;
            imgPhotographerHomePage.setAttribute("title",`photo de ${photographer.name}`);
